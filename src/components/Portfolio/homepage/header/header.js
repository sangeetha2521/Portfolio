...
export default function Header() {
  ...
  return (
    <><header className="flex items-center justify-between p-4 md:p-6 bg-gray-100">
      {/* Theme toggle button */}
      ...
      {/* Menu */}
      <nav
        className={`${isOpen ? "block" : "hidden"} md:block w-full md:max-h-1/2 mt-4 md:mt-0 md:flex md:items-center md:w-auto`}
      >
        {isOpen && (
        ...
        <ul className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-5">
          {menu.map((item, index) => (
            <li key={index}>
              <Link
                to={`/${item.key}`}
                className={`${activeItem === item.key ? "text-blue-500" : "text-gray-500 hover:text-blue-500"}`}
